...
public class SystemOperatorService extends BaseService {
    ...
    public BaseResult<HashMap<String,Object>> login(String operatorAccount, String operatorPwd, Integer operatorType, Integer seniorOperatorId) {
        ...
        try {
            ...
            operator = systemOperatorProxy.getOperatorByAccount(operatorAccount, operatorType, seniorOperatorId);
            if (operator == null) {
    ...
    public BaseResult<HashMap<String,Object>> register(String operatorAccount, String operatorPwd, Integer seniorOperatorId) {
        try {
            SystemOperator operator = systemOperatorProxy.getOperatorByAccount(operatorAccount, 2, seniorOperatorId);
            if (operator !=null && operator.getOperatorAccount().equals(operatorAccount)) {
                return errorResult("账户已存在");
            } else {
                operator = new SystemOperator();
            }
            ...
            operator.setSeniorOperatorId(seniorOperatorId);
            systemOperatorProxy.saveSystemOperator(operator);
            return successResult(200, "success", new HashMap<>() );
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public BaseResult<HashMap<String,Object>> addPopularity(Integer blogPopularityId) {
        try {
            systemOperatorProxy.updateBlogPopularity(blogPopularityId);
