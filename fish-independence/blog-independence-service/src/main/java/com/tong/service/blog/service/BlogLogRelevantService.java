...
public class BlogLogRelevantService extends BaseService {
    ...
    public BaseResult<List<TagInfo>> getTagList(Integer operatorId, Integer categoryInfoId, String searchText) {
        try {
            // 返回结果列表
            List<TagInfo> tagInfoList = blogLogRelevantProxy.getTagList(operatorId, categoryInfoId, searchText);
            return successResult("数据获取成功", tagInfoList);
        }catch (Exception e) {
            e.printStackTrace();
    ...
    public BaseResult<HashMap<String,Object>> saveArticleInfo(ArticleInfo articleInfo) {
        try {
            ...
            return successResult("文章信息保存成功！", result);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public BaseResult<ArticleStatistics> getArticleStatistics(Integer seniorOperatorId) {
        try {
            // 返回结果
            ArticleStatistics articleStatistics = blogLogRelevantProxy.getArticleStatistics(seniorOperatorId);
    ...
    public BaseResult<List<ArticleLine>> getArticleLine(Integer seniorOperatorId) {
        try {
            // 返回结果
            List<ArticleLine> articleLine = blogLogRelevantProxy.getArticleLine(seniorOperatorId);
    ...
    public BaseResult saveDeleteImageInfo(String imagePath) {
        try {
            if (imagePath != null && StringUtil.isNotEmpty(imagePath)) {
                String[] imageArr = imagePath.split(",");
                blogLogRelevantProxy.saveDeleteImageInfo(imageArr);
            }
            return successResult("成功保存删除的图片信息");
        }catch (Exception e) {
            e.printStackTrace();
